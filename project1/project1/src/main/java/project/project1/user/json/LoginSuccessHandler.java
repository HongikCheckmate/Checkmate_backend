...
public class LoginSuccessHandler extends SimpleUrlAuthenticationSuccessHandler  {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        String username = extractUsername(authentication); // 인증 정보에서 Username(id) 추출
        SiteUser siteuser = userRepository.findByUsername(username)
                .orElseThrow(() -> new IllegalArgumentException("유저를 찾을 수 없음"));

        String accessToken = jwtService.createAccessToken(siteuser.getId(), username,siteuser.getRole().getKey(), siteuser.getNickname()); // JwtService의 createAccessToken을 사용하여 AccessToken 발급
